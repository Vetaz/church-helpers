 ...
 */
function addManualAttendance(dateText: string, rawNames: string): void {
  ...
  document.querySelectorAll<HTMLTableRowElement>('tbody:first-of-type > tr').forEach((rowElement) => {
    const [nameCell, _gender, ...dateCells] = rowElement.querySelectorAll<HTMLTableCellElement>('td')
...
function getMostRecentSunday(): Temporal.PlainDate {
