...
export const hasCheckMark = (childNode) => childNode.querySelector('path')?.getAttribute('d') ===
...
export const getDateText = (number) => document.querySelectorAll('thead > tr > th')[number + 2]?.innerText;
const monthSelector = document.querySelectorAll('select')[1];
export const changeMonthView = (dateText) => {
    const yearMonthStr = Temporal.PlainYearMonth.from(parseDayMonthYear(dateText)).toString();
    if (!monthSelector)
        throw new Error('Missing month selector.');
    const option = Array.from(monthSelector.options).find(({ value }) => value === yearMonthStr);
    if (!option)
        throw new Error(`Month ${yearMonthStr} was not found in month selector dropdown.`);
